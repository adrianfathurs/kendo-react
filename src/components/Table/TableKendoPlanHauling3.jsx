...
import { cellFontColorA2B, cellWithBackGroundA2BSupport, cellFontColorPlanHauling} from "../../utils/table";
...
const mergeCells = (data) => {
  ...
  for (let i = 0; i < data.data.length; i += looped) {
    ...
    for (let j = i + 1; j < data.data.length; j++) {
      if (data.data[i].entity === data.data[j].entity) {
        looped++;
        rowSpan++;
      } else {
        break;
...
const TableKendoPlanHauling3 = ({dataTableProps, columnsDataTable, limitMinimumProps}) => {
  ...
  const [stateColumns, setStateColumns] = React.useState(columnsDataTable);
        ...
        {stateColumns.map(
          (column, idx) =>
            column.show && column.children.length == 0 ? (
              <Column
                key={idx}
                cell={ column.title !== "Entity" && column.field ? (props) => cellFontColorPlanHauling(props) : null}
                field={column.field}
                title={column.title}
                ...
                <Column title={column.title}>
                  {
                    column.children.map((item)=>(
                      <Column
                        key={idx}
                        cell={(props) => cellFontColorPlanHauling(props)}
                        field={item.field}
                        title={item.title}
                        filter={item.filter}
                        // columnMenu={(props) => (
...
export default TableKendoPlanHauling3;
